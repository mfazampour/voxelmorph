...
def setBoxColors(bp, colors: List[str] = None):
    if colors is None:
        colors = ['b', 'r', 'k']
...
def boxplot_scores(data_list, tags, legends, pre, ax, title: str, on_top=False):  #: List[Dict[str: np.ndarray]]
    ...
    for i, tag in enumerate(tags):
        ...
        for d in data_list:
            data.append(d[tag])
        bp = ax.boxplot(data, positions=[pos + 1, pos + 2, pos + 3], widths=0.6, showfliers=False)
...
def draw_boxplot(filenames: List[str], pre_reg_value, ax, title: str, on_top=False):
    data1, tags1 = read_csv_result(filenames[0])
    data2, _ = read_csv_result(filenames[1])
    data3 = read_csv_result_vert(filenames[2])

    # plot the box plot using the data list and the tags
    boxplot_scores([data1, data2, data3], tags1,
                   legends=('SG-MCMC', 'VI', 'VoxelMorph', 'affine'),
...
def read_csv_result_vert(filename):
    with open(filename, newline='') as csvfile:
        ...
        info.__next__()  # first row is background
        data3 = {}
        for i, row in enumerate(info):
            data3[row[0].replace('_', ' ')] = np.asarray(row[1:]).astype(np.float)
    return data3
...
if __name__ == "__main__":
    ...
    filenames = ['../../results/SGMCMC_ASD.csv', '../../results/VI_ASD.csv', '../../results/vxm_asd.csv']
    ...
    draw_boxplot(filenames, pre_reg_value=asd_pre_registration, ax=ax1, title='average surface distance (mm)', on_top=True)

    filenames = ['../../results/SGMCMC_DSC.csv', '../../results/VI_DSC.csv', '../../results/vxm_dice.csv']
    ...
    draw_boxplot(filenames, pre_reg_value=dsc_pre_registration, ax=ax2, title='Dice score')
